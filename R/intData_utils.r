...
#' 
#' @details
#' The latent variables, \eqn{U_{ij}}, are defined according to the following model:
#' 
#' Let \eqn{X_j=(C_j,R_j)^\top=\left[C_j-\dfrac{R_j}{2}, C_j+\dfrac{R_j}{2}\right]} represent the \strong{macrodata} and
#' \deqn{V_{ij}=C_j+U_{ij}\dfrac{R_j}{2},\quad j=1,\dots,p,\ i=1,\dots,m_j}
#' the \strong{microdata} with \eqn{U_{ij}} being random variables with support on \eqn{[-1,1]}, uncorrelated with \eqn{(C_j,R_j)}.
#' 
#' @param microdata A matrix containing the microdata.
#' @param macrodata A data frame, matrix or \linkS4class{intData} object containing the macrodata/interval data.
...
#'      \itemize{
#'         \item \eqn{[\boldsymbol{\mathfrak{E}}_{UU}]_{ij}=\mathcal{E}(U_i,U_j)}, \eqn{i\neq j}, with \eqn{\mathcal{E}(U_i,U_j)=\int_0^1 F_{U_i}^{-1}(t) F_{U_j}^{-1}(t) \, dt}, and \eqn{[\boldsymbol{\mathfrak{E}}_{UU}]_{ii}=\mathbb{E}(U_i^2)}, \eqn{i,j=1,\dots,p}
...
get_latent_param <- function(LatentCase=c("U_id_symmetric","U_id","General"),
    ...
    if (!identical(LatentDist, c("Unif","Triang","TNorm","InvTri","Beta","KDE","Degenerated"))){dist <- match.arg(LatentDist,several.ok = TRUE)}
        ...
        if(case=="General") stop("Error: a distribution type must be provided.")
    ...
    if(case=="General"&&estimate.DistParam){
        ...
        for(i in 1:p){
            TriangParam[i]<-3*mean(Umicro[,i],na.rm=TRUE)
    ...
    if(case=="U_id_symmetric"){
        if(dist=="InvTri")      {delta <- 1/8}
            ...
            print("The latent variable, U, is a degenerated random variable, i.e., P(U=0)=1 and the data is considered as non-symbolic")
            delta <- 0
        }else{
    ...
    return(list(LatentParam=param,TriangParam=TriangParam,BetaParam.a=BetaParam.a,BetaParam.b=BetaParam.b))
}
...
CalE.beta.beta <- function(a1,b1,a2,b2){
    ...
    #calE11 <- 4*a1*b1/( (a1+b1)^2*(a1+b1+1) )+(2*a1/(a1+b1)-1)^2
    #calE22 <- 4*a2*b2/( (a2+b2)^2*(a2+b2+1) )+(2*a2/(a2+b2)-1)^2
