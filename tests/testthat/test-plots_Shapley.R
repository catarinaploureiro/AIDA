...
test_that("tileplot_int_Shapley and radarplot_int_Shapley run", {
  testthat::skip_if_not_installed("ggplot2")
  shp <- make_shapley_matrix(4, 3)
  expect_s3_class(tileplot_int_Shapley(shp), "ggplot")

  # radarplot uses base graphics and fmsb
  testthat::skip_if_not_installed("fmsb")
  expect_error(with_null_device(radarplot_int_Shapley(shp)), NA)
})

test_that("beeswarm_int_Shapley and barplot_decomp produce plots when deps present", {
  testthat::skip_if_not_installed("ggplot2")
  ...
  res <- beeswarm_int_Shapley(shp, color_class = color_class, ggplotly = FALSE)
  expect_s3_class(res, "ggplot")

  # barplot_int_Shapley_decomp expects a list of matrices
...
test_that("beeswarm_int_Shapley ggplotly returns plotly object", {
  testthat::skip_if_not_installed("ggplot2")
  testthat::skip_if_not_installed("plotly")
  shp <- make_shapley_matrix(5, 3)
  color_class <- rep(c("Regular","Outlier"), length.out = nrow(shp))
  res_plotly <- beeswarm_int_Shapley(shp, color_class = color_class, ggplotly = TRUE)
...
test_that("plots error on clearly invalid inputs", {
  ...
  expect_error(barplot_int_Shapley(list(1,2)))
  expect_error(tileplot_int_Shapley(1:5))
  expect_error(radarplot_int_Shapley(list(1,2)))
  expect_error(beeswarm_int_Shapley(1:5))
  expect_error(barplot_int_Shapley_decomp(matrix(1:9, 3)))
})
...
test_that("tileplot_int_Shapley highlights outliers and show_values", {
  ...
  p <- tileplot_int_Shapley(shp, outliers = outliers, show_values = TRUE, sort.var = TRUE, sort.obs = TRUE)
  expect_s3_class(p, "ggplot")
  expect_true(length(p$layers) >= 1)
})

test_that("radarplot_int_Shapley draws without error for sorted observations", {
  testthat::skip_if_not_installed("fmsb")
  shp <- make_shapley_matrix(4, 3)
  expect_error(with_null_device(radarplot_int_Shapley(shp, palette = c("red", "blue", "green", "purple"), sort.obs = TRUE)), NA)
})

test_that("beeswarm_int_Shapley supports ggplotly = FALSE and shape labels", {
  testthat::skip_if_not_installed("ggplot2")
  testthat::skip_if_not_installed("ggbeeswarm")
  ...
  p <- beeswarm_int_Shapley(shp, color_class = color_class, shape_class = shape_class,
...
test_that("barplot_int_Shapley_decomp can plot IMah cutoff lines", {
  testthat::skip_if_not_installed("ggplot2")
  decomp <- list(obs1 = matrix(1:6, nrow = 2, byrow = TRUE), obs2 = matrix(2:7, nrow = 2, byrow = TRUE))
  colnames(decomp[[1]]) <- colnames(decomp[[2]]) <- c("Centers", "Ranges", "CentersRanges")[seq_len(ncol(decomp[[1]]))]
  rownames(decomp[[1]]) <- rownames(decomp[[2]]) <- c("X", "Y")
