...
The `intData` class is used to create objects that represent interval-valued data. The `int_Shapley` function computes the Shapley value for each observation, based on the robust squared Interval-Mahalanobis distances. The `int_Shapley_interaction` function computes the Shapley interaction values for pairs of variables. The `int_Shapley_decomp` function decomposes the Shapley values into centers, ranges, and their interactions' contributions. The functions `barplot_int_Shapley`, `beeswarm_int_Shapley`, `tileplot_int_Shapley`, and `radarplot_int_Shapley` are used to visualize the Shapley values in different ways. The functions `plot_int_Shapley_inter` and `barplot_int_Shapley_decomp` are used to visualize the Shapley interaction values and the decomposition of the Shapley values, respectively. The examples provided here demonstrate how to apply these methods to real datasets, and the results can be compared to those presented in the paper for validation.
